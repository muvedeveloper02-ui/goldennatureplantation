...
type CartItem = {
  ...
  name: string;
  image: string;
  price?: number;          // legacy field (LKR)
  basePriceLkr?: number;   // new field from ProductDetails
  quantity: number;
};
...
const Cart = () => {
  ...
  const { formatPrice, currency } = useCurrency(); // ✅ global currency + formatter
  ...
  // Safely get base price in LKR for each item
  ...
  // Work in LKR internally, then format with selected currency
  const grandTotalLkr = cart.reduce(
    (total, item) =>
      total + getItemBasePriceLkr(item) * (item.quantity || 0),
    0
  );
      ...
      <section className="py-24">
        <div className="container mx-auto px-4 lg:px-8 max-w-4xl bg-white p-10 rounded-2xl shadow-elevated">
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-display text-3xl">Shopping Cart</h2>
